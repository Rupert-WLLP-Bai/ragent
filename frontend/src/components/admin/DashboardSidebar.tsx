...
import type { DashboardOverview, DashboardPerformance } from "@/services/dashboardService";

export type HealthStatus = "healthy" | "attention" | "critical";
export type MetricTone = "good" | "warning" | "bad";

export type MetricStatusView = {
  success: MetricTone;
  latency: MetricTone;
  error: MetricTone;
  noDoc: MetricTone;
...
const TYPE_ICON: Record<InsightCardData["type"], LucideIcon> = {
  anomaly: AlertCircle,
  trend: Info,
          ...
          return (
            <div key={item.label} className="space-y-1.5">
              <div className="flex h-24 items-end rounded-md border border-slate-200 bg-white p-1.5">
                <div
                  className={cn("w-full rounded-sm transition-[height] duration-500", item.toneClass)}
                  style={{ height: barHeight }}
                />
              </div>
              <div className={cn("text-center text-xs font-semibold tabular-nums", item.valueClass)}>
  ...
  return (
    <div className="mt-4 rounded-xl border border-slate-100 bg-slate-50 p-3.5">
      ...
      <div className="divide-y divide-slate-100">
        {metrics.map((metric) => {
          const valueText = metric.value === null ? "-" : `${formatRatio(metric.value)} ${metric.unit}`;
          return (
  ...
  return (
    <DashCard>
      <div className="mb-4 flex items-center justify-between">
        ...
        <span className={cn("rounded-full px-2.5 py-1 text-xs font-medium", healthCfg.bg, healthCfg.text)}>
          {healthCfg.label}
        </span>
      </div>
...
const InsightCard = ({ item }: { item: InsightCardData }) => {
  ...
  return (
    <div className="rounded-xl bg-slate-50 p-3.5">
      ...
      {item.action ? <p className="mt-1 text-xs font-medium text-slate-600">建议：{item.action}</p> : null}
    </div>
  );
};
...
const InsightSection = ({
  ...
  return (
    <DashCard className={cn("flex flex-col", className)}>
      <h3 className="mb-4 text-sm font-semibold text-slate-700">运营洞察</h3>
      <div
        ref={contentRef}
...
export function DashboardSidebar({
  ...
  return (
    <aside className={cn("space-y-5 xl:sticky xl:top-4 xl:self-start", className)}>
      <AIPerformanceCard
